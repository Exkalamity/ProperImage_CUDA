...
class SingleImage(object):
    ...
    def n_sources(self):
        ...
        try:
            ...
            print(time.time() - start, "stamps_pos")
            return self._n_sources
    ...
    def cov_matrix(self):
        ...
        if not hasattr(self, "_covMat"):
            ...
            covMat = np.zeros(shape=(self.n_sources, self.n_sources)) #time consumer
            print(time.time() - start, "zeros")
            if self.n_sources <= 250:
                ...
                for i in range(self.n_sources):
                    ...
                    #print(time.time() - start, "flatten")
                #start = time.time()              
                covMat = np.cov(m, rowvar=False)
                #print(time.time() - start, "cov")
                ...
                for i in range(self.n_sources):
                    ...
                    for j in range(self.n_sources):
                        if i <= j:
                            ...
                            covMat[j, i] = inner
            self._covMat = covMat
        print(time.time() - start_function, "function")
        return self._covMat
    ...
    def interped(self):
        ...
        if not hasattr(self, "_interped"):
            ...
            img_interp = np.nan_to_num(img, nan=np.nanmean(img))#interpolate_replace_nans(img, kernel, convolve=conv) #big time consumer
